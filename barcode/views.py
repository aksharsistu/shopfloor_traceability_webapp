...
def index(request):
    ...
    if Trace.objects.all().filter(sno__exact=data['barcode']).filter(stage_id__exact=data['stage']).exists() and not \
            data['override']:
    ...
    q = Trace(username=UserData.objects.get(username__exact=data['username']), sno_id=data['barcode'],
              stage=StageData.objects.get(line_code__exact=data['stage']), time=timezone.now(),
...
def check_previous_stages(data):
    ...
    for i in range(0, index_val):
        if not Trace.objects.all().filter(sno__exact=data['barcode']).filter(stage_id__exact=arr[i]).filter(product_name_id__exact=data['product']):
            return arr[i]
    return 0
